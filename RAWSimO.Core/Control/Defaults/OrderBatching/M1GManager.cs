...
namespace RAWSimO.Core.Control.Defaults.OrderBatching
{
    ...
    public class M1GManager : OrderManager
    {
                ...
                HashSet<Pod>> inboundPods, HashSet<Bot> Ra, HashSet<Bot> Rb, HashSet<Bot> R, HashSet<Pod> Pb, HashSet<Pod> Pa, Dictionary<Pod, Bot> PodToBot)
        {
            ...
            //    }
            //}
            foreach (var station in Cs.Keys) //订单中的所有SKU i 必须被满足
            {
                foreach (var order in pendingOrders.Where(o => o.Positions.All(p => Instance.StockInfo.GetActualStock(p.Key) >= p.Value)))//订单不可分割
                {
                    IEnumerable<KeyValuePair<ItemDescription, int>> ListofSKUID = order.Positions;
                    foreach (var skuid in ListofSKUID)
                    {
                        wrapper.AddConstr(LinearExpression.Sum(deVarNameziops.Where(v => v.skui.ID == skuid.Key.ID && v.order.ID == order.ID && v.outputstation.
                        ID == station.ID).Select(v => variablesInteger2[v.name])) == skuid.Value * variablesBinary["yos" + "_" + order.ID.ToString() + "_" + station.ID.ToString()], "shi8");
                    }
                }
            }
