...
namespace RAWSimO.Core.Control.Defaults.OrderBatching
{
    ...
    public class M1GManager : OrderManager
    {
        ...
        public Dictionary<OutputStation, HashSet<Pod>> GeneratePs(Dictionary<OutputStation, int> Cs)
        {
            ...
            {
                inboundPods.Add(station, station.InboundPods.ToHashSet());
            ...
            IEnumerable<Pod> allPods, HashSet<Order> pendingOrders, Dictionary<OutputStation, int> Cs, HashSet<Bot> R, HashSet<Pod> Pa1, out HashSet<Pod> Pa)
        {
            ...
            variableNames.Add(7, deVarNamedops);
            ...
            variableNames.Add(5, deVarNameyrp);
            ...
            variableNames.Add(6, deVarNameus);
            Pa = new HashSet<Pod>();
                ...
                HashSet<Pod>> inboundPods, HashSet<Bot> Ra, HashSet<Bot> Rb, HashSet<Bot> R, HashSet<Pod> Pb, HashSet<Pod> Pa, Dictionary<Pod, Bot> PodToBot)
        {
            ...
            List<Symbol> deVarNameyrp = variableNames[5];
            List<Symbol> deVarNameus = variableNames[6];
            List<Symbol> deVarNamedops = variableNames[7];
            double w1 = 1;
            double w2 = -40;
            double w3 = 10000;
            //double w4 = 2;
            ...
            //if (deVarNamedops.Where(u => Pb.Contains(u.pod)).Select(v => variablesBinary[v.name]).Count() > 0)
            //{
            //    if (Ra.Count() > 0)
            //        wrapper.SetObjective((LinearExpression.Sum(deVarNamedops.Where(u => Pa.Contains(u.pod)).Select(v => variablesBinary[v.name] * DistanceSet[v.outputstation.Waypoint.ID][v.podwaypointID] * w4))
            //            + LinearExpression.Sum(deVarNamedops.Where(u => Pb.Contains(u.pod)).Select(v => variablesBinary[v.name] * DistanceSet[v.outputstation.Waypoint.ID][v.podwaypointID]))
            //            + LinearExpression.Sum(deVarNameyrp.Where(u => Ra.Contains(u.robot) && Pa.Contains(u.pod)).Select(v => variablesBinary[v.name] *
            ...
            //    else
            //        wrapper.SetObjective((LinearExpression.Sum(deVarNamedops.Where(u => Pa.Contains(u.pod)).Select(v => variablesBinary[v.name] * DistanceSet[v.outputstation.Waypoint.ID][v.podwaypointID] * w4))
            //            + LinearExpression.Sum(deVarNamedops.Where(u => Pb.Contains(u.pod)).Select(v => variablesBinary[v.name] * DistanceSet[v.outputstation.Waypoint.ID][v.podwaypointID]))) * w1
            ...
            //{
            //    if (Ra.Count() > 0)
            //        wrapper.SetObjective((LinearExpression.Sum(deVarNamedops.Where(u => Pa.Contains(u.pod)).Select(v => variablesBinary[v.name] * DistanceSet[v.outputstation.Waypoint.ID][v.podwaypointID])) +
            //            LinearExpression.Sum(deVarNameyrp.Where(u => Ra.Contains(u.robot) && Pa.Contains(u.pod)).Select(v => variablesBinary[v.name] *
            //            Distances.CalculateManhattan1(v.robot.CurrentWaypoint, v.pod.Waypoint)))) * w1 + LinearExpression.Sum(deVarNameyos.Select(v => variablesBinary[v.name])) * w2
            //            + LinearExpression.Sum(deVarNameus.Select(v => variablesInteger3[v.name])) * w3, OptimizationSense.Minimize);
            //    else
            //        wrapper.SetObjective(LinearExpression.Sum(deVarNamedops.Where(u => Pa.Contains(u.pod)).Select(v => variablesBinary[v.name] * DistanceSet[v.outputstation.Waypoint.ID][v.podwaypointID])) * w1
            //            + LinearExpression.Sum(deVarNameyos.Select(v => variablesBinary[v.name])) * w2
            //            + LinearExpression.Sum(deVarNameus.Select(v => variablesInteger3[v.name])) * w3, OptimizationSense.Minimize);
            //}
            if (Ra.Count() > 0)
                wrapper.SetObjective((LinearExpression.Sum(deVarNamedops.Select(v => variablesBinary[v.name] * DistanceSet[v.outputstation.Waypoint.ID][v.podwaypointID]))
                ...
                wrapper.SetObjective(LinearExpression.Sum(deVarNamedops.Select(v => variablesBinary[v.name] * DistanceSet[v.outputstation.Waypoint.ID][v.podwaypointID])) * w1
                    + LinearExpression.Sum(deVarNameyos.Select(v => variablesBinary[v.name])) * w2
                    + LinearExpression.Sum(deVarNameus.Select(v => variablesInteger3[v.name])) * w3, OptimizationSense.Minimize);
            foreach (var order in pendingOrders)//订单不可分割
            {
                IEnumerable<KeyValuePair<ItemDescription, int>> ListofSKUID = order.Positions;
                foreach (var station in Cs.Keys)
                {
                    foreach (var skuid in ListofSKUID)
                    {
                        wrapper.AddConstr(variablesBinary["yios" + "_" + skuid.Key.ID.ToString() + "_" + order.ID.ToString() + "_" + station.ID.ToString()] == variablesBinary
                            ["yos" + "_" + order.ID.ToString() + "_" + station.ID.ToString()], "shi2");
                    }
                }
            }
            ...
            foreach (var order in pendingOrders)//当订单分配给工作站时，订单一定能够被分配给工作站
            {
                foreach (var station in Cs.Keys)
                    wrapper.AddConstr(variablesBinary["yaos" + "_" + order.ID.ToString() + "_" + station.ID.ToString()] <= variablesBinary
                        ["yos" + "_" + order.ID.ToString() + "_" + station.ID.ToString()], "shi4");
            }
            foreach (var station in Cs.Keys)//分派到工作站的订单数量必须等于工作站的可利用容量减去未被利用的容量
                wrapper.AddConstr(LinearExpression.Sum(deVarNameyaos.Where(v => v.outputstation.ID == station.ID).Select(v => variablesBinary[v.name])) == Cs[station] - variablesInteger3["us" + "_" + station.ID.ToString()], "shi5");            
            foreach (var sku in OiSKU.Where(v => PiSKU.ContainsKey(v.Key)))//由分配给工作站w的货架p满足订单o中SKU i 数量不能大于货架p中存储的SKU i 的数量
            {
                ...
                foreach (var pod in listofpod)
                {
                    foreach (var instance in Cs.Keys)
                    {
                        ...
                        variablesInteger2[v.name])) <= pod.CountAvailable(sku.Key) * variablesBinary["xps" + "_" + pod.ID.ToString() + "_" + instance.ID.ToString()], "shi7");
                    }
                }
            }
            foreach (var sku in OiSKU.Where(v => PiSKU.ContainsKey(v.Key)))//订单中的所有SKU i 必须被满足
            {
                foreach (var station in Cs.Keys)
                {
                    foreach (var order in sku.Value)
                    {
                        ...
                        ID == station.ID).Select(v => variablesInteger2[v.name])) == order.GetDemandCount(sku.Key) * variablesBinary["yios" + "_" + sku.
                        Key.ID.ToString() + "_" + order.ID.ToString() + "_" + station.ID.ToString()], "shi8");
                    }
                }
            }
            foreach (var pod in Pods)//货架最多只能分配给1个工作站
                wrapper.AddConstr(LinearExpression.Sum(deVarNamexps.Where(v => v.pod.ID == pod.ID).Select(v => variablesBinary[v.name])) <= 1, "shi9");
            foreach (var station in inboundPods)
            {
                foreach (var pod in station.Value)
                {
                    wrapper.AddConstr(variablesBinary["xps" + "_" + pod.ID.ToString() + "_" + station.Key.ID.ToString()] == 1, "shi10");//继承系统中已经分配而未释放的货架
                    wrapper.AddConstr(variablesBinary["yrp" + "_" + PodToBot[pod].ID.ToString() + "_" + pod.ID.ToString()] == 1, "shi16");//继承系统中已经分配的机器人
                }
            }           
            foreach (var pod in Pods)//当货架被分配给工作站时，必须有对应的机器人分配给货架。
                wrapper.AddConstr(LinearExpression.Sum(deVarNamexps.Where(v => v.pod.ID == pod.ID).Select(v => variablesBinary[v.name])) <= LinearExpression.Sum(deVarNameyrp.Where(v => v.pod.ID == pod.ID).Select(v => variablesBinary[v.name])), "shi11");
            foreach (var pod in Pods)//每个货架最多只能被一个机器人运输
                wrapper.AddConstr(LinearExpression.Sum(deVarNameyrp.Where(v => v.pod.ID == pod.ID).Select(v => variablesBinary[v.name])) <= 1, "shi12");
            foreach (var robot in R)//每个机器人最多只能分配给一个货架
                wrapper.AddConstr(LinearExpression.Sum(deVarNameyrp.Where(v => v.robot.ID == robot.ID).Select(v => variablesBinary[v.name])) <= 1, "shi13");
            //foreach (var robot in Ra)//空闲的机器人不能分配给运输中的货架
            //{
            //    foreach (var pod in Pb)
            //        wrapper.AddConstr(variablesBinary["yrp" + "_" + robot.ID.ToString() + "_" + pod.ID.ToString()] == 0, "shi14");
            //}
            //foreach (var robot in Rb)//被占用的机器人不能分配给货架，除了其正在运输的货架
            //{
            //    foreach (var pod in Pods.Where(v => v != PodToBot.Where(u => u.Value.ID == robot.ID).FirstOrDefault().Key)) 
            //        wrapper.AddConstr(variablesBinary["yrp" + "_" + robot.ID.ToString() + "_" + pod.ID.ToString()] == 0, "shi15");
            //}
            //foreach (var order in pendingOrders)//当yaos等于0时，dops一定等于0
            //{
            //    foreach (var pod in Pods)
            //    {
            //        foreach (var station in Cs.Keys)
            //        {
            //            wrapper.AddConstr(variablesBinary["dops" + "_" + order.ID.ToString() + "_" + pod.ID.ToString() + "_" + station.ID.ToString()]
            //                <= variablesBinary["yaos" + "_" + order.ID.ToString() + "_" + station.ID.ToString()], "shi17");
            //        }
            //    }
            //}
                    ...
                    foreach (var order in sku.Value)
                    {
                        foreach (var station in Cs.Keys)
                        {
                            wrapper.AddConstr(variablesBinary["dops" + "_" + order.ID.ToString() + "_" + pod.ID.ToString() + "_" + station.ID.ToString()] 
                                <= variablesBinary["yaos" + "_" + order.ID.ToString() + "_" + station.ID.ToString()], "shi17");
                            wrapper.AddConstr(variablesBinary["dops" + "_" + order.ID.ToString() + "_" + pod.ID.ToString() + "_" + station.ID.ToString()]
                            <= LinearExpression.Sum(deVarNameziops.Where(v => v.pod.ID == pod.ID && v.order.ID == order.ID && v.outputstation.ID == station.ID).Select(v => variablesInteger2[v.name])), "shi18");
                            //wrapper.AddConstr(variablesBinary["yaos" + "_" + order.ID.ToString() + "_" + station.ID.ToString()] + 0.1 * variablesInteger2["ziops" + "_" + sku.Key.ID.ToString() + "_" + order.ID.ToString() + "_" + pod.ID.ToString() + "_" + station.ID.ToString()]
                            //    <= 1 + variablesBinary["dops" + "_" + order.ID.ToString() + "_" + pod.ID.ToString() + "_" + station.ID.ToString()], "shi19");
                        }
                    }
                }
            }
            foreach (var pod in Pa)//保证新分配的pod中最少有一个对应的订单分配
            {
                foreach (var station in Cs.Keys)
                    wrapper.AddConstr(variablesBinary["xps" + "_" + pod.ID.ToString() + "_" + station.ID.ToString()]
                        <= LinearExpression.Sum(deVarNamedops.Where(v => v.pod.ID == pod.ID && v.outputstation.ID == station.ID).Select(v => variablesBinary[v.name])), "shi20");
            }
            wrapper.Update();
            wrapper.Optimize();
            wrapper.ExportLP("lpexl.lp");          
            //StationQueueClear(Cs);//每次决策前对queue中的order进行归零
            //Instance.ResourceManager.BottoPod.Clear();
            if (wrapper.HasSolution())
            {
                ...
                for (int i = 1; i < variableNames.Count + 1; i++)
                {
                    ...
                    foreach (var itemName in variableName)
                    {
                        ...
                        {
                            int ziops = (int) Math.Round(variablesInteger2[itemName.name].GetValue());
                            //if(!Pods.Contains(itemName.pod))
                            //    Thread.Sleep(1);
                            NewZiops.Add(itemName, ziops);
                            ...
                            Instance.ResourceManager._Ziops[itemName.outputstation].Add(itemName, ziops);
                        }
                        else if (i == 5 && Math.Round(variablesBinary[itemName.name].GetValue()) != 0)
                        {
                ...
                //if (IsdeVarNameyaos.Count() == 0 && Ra.Count() > 2)
                //{
                //    foreach (var order in pendingOrders)
                //    {
                //        _pendingOrders.Remove(order);
                //        Instance.ItemManager.DeleteOrder(order);
                //    }
                //}
                //_IsvariableNames.Add(0, IsdeVarNameyrp);
                //将Instance.ResourceManager._Ziops中多余的item去掉 因为_Ziops中标记的是能够被分配的订单，而仿真实际需要的是此次决策被分配的订单
                if (IsdeVarNameyos.Count != IsdeVarNameyaos.Count)
                {
                    ...
                    foreach (var itemName in deleteyos)
                    {
                        ...
                        //Instance.Controller.Allocator.Queue(itemName.order, itemName.outputstation);
                        //Instance.ResourceManager._QueueZiops[itemName.outputstation].Add(itemName, NewZiops[itemName]);
